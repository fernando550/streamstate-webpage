...
const Description = (props) => {
  return <div className="z-depth-5 p-4 bg-white rounded-lg">{props.text}</div>;
};

const TweetHist = (props) => {
  return (
    <div className="p-3 rounded-lg z-depth-5 bg-white">
      ...
      <div
        ...
        style={{
          marginTop: "10px",
          // display: props.loading === "tweet-parser" ? "block" : "none",
        }}
      >
  ...
  return (
    <div className="p-3 rounded-lg z-depth-5 bg-white">
      ...
      <div
        ...
        style={{
          marginTop: "10px",
          display: props.loading === "user-parser" ? "block" : "none",
        }}
      >
        <p>user input: {props.userName}</p>
        <p>{props.parseFriend}</p>
...
class Functions extends Component {
    ...
    try {
      await Papa.parse(this.state.fileUpload, {
        complete: async (results) => {
          ...
          console.log(
            "aggregating list of mutual ",
            this.state.parserType,
            " by IDs"
          );
          ...
          console.log(
            "aggregated list not empty?: ",
            outputUsers.length > 0 ? true : false,
            "\n list length: ",
            outputUsers.length
          );
          console.log(
            "Retrieving user data from IDs list of mutual " +
              this.state.parserType +
              " ..."
          );
          this.setState({
            wrapUp:
              "wrapping up, please wait approximately: " +
              Math.floor(outputUsers.length / 2000) +
              " minutes ",
          });
  ...
  processUser = async () => {
    try {
      ...
      if (this.state.parserType === "friends") {
        ...
        initResponse = await axios.post("/ttapi/getFriendIDs", {
        ...
        initResponse = await axios.post("/ttapi/getFollowerIDs", {
          data: this.state.userName,
        });
        initData = initResponse.data;
        if (initData.length > 0) {
  ...
  getUserTweets = async (username) => {
    ...
    do {
      try {
        ...
        const C = await timer.set(2000).then(async () => {
          ...
          this.setState({
            parseFriend:
              "Processing up to 3200 tweets; currently have processed: " +
              String(i - 1) +
              " tweets",
            progressBar:
              Math.floor((String(i - 1) / 3200) * 100).toString() + "%"
          });
    ...
    const tweetHist = [];
    result.forEach((array) => array.forEach((user) => tweetHist.push(user)));
    return tweetHist;
  };

  getUserNames = async (userids) => {
    console.log("entered getUserNames Function")
    var promiseArray = [];
    var i = 0;
    ...
    do {
      try {
        ...
        const B = await timer.set(61000).then(async () => {
          const A = await axios.post("/ttapi/getFollowerIDs", {
            data: usernames[listArray[i]],
          });
          this.setState({
            parseFriend:
              "processing user " +
              String(i + 1) +
              " out of " +
              this.state.userListLength +
              " ...",
            progressBar:
              Math.floor(
                (String(i + 1) / this.state.userListLength) * 100
              ).toString() + "%"
          });
          var end = Date.now() - start;
          console.log(
  ...
  getFollowerIDs2 = async (username) => {
    ...
    do {
      try {
        ...
        const B = await timer.set(61000).then(async () => {
          const A = await axios.post("/ttapi/getFollowerIDs2", {
            screenname: username,
            cursor: cursor,
          });
          ...
          this.setState({
            parseFriend:
              ...
              " out of 50,000 max followers ...",
            progressBar:
              Math.floor((String(i + 1) / 10) * 100).toString() + "%"
          });
          var end = Date.now() - start;
          console.log(
            "Elapsed time: ",
  ...
  getFriendIDs = async (usernames) => {
    ...
    do {
      try {
        ...
        var start = Date.now();
        const B = await timer.set(61000).then(async () => {
          const A = await axios.post("/ttapi/getFriendIDs", {
            data: usernames[listArray[i]],
          });
          this.setState({
            parseFriend:
  ...
  getFriendIDs2 = async (username) => {
    ...
    do {
      try {
        ...
        console.log("current cursor: ", cursor);
        const B = await timer.set(61000).then(async () => {
          const A = await axios.post("/ttapi/getFriendIDs2", {
            screenname: username,
            cursor: cursor,
          });
          cursor = A.data.cursor;
  ...
  getUserDataInput = async () => {
    try {
      this.setState({
        ...
        disableButton: true,
        outputFileName:
          "User_" + this.state.parserType + " " + this.state.userName,
      });
      ...
      this.setState({
        wrapUp:
          "wrapping up, please wait approximately: " +
          Math.floor((userids.length * 0.03) / 60) +
          " minutes ",
      });
          ...
          item.id === "tweetView" && item.view === true ? (
            [
              <Description
                text={
                  <p>
                    info: Enter a user handle to receive up to the latest 3200
                    tweets with tweet data such as location, time, and retweet
                    count.
                  </p>
                }
              />,
            ...
            [
              <Description
                text={
                  <div>
                    <p>
                      info: This function will find mutual followers and friends
                      amongst a group of people. This means you will be able to
                      speculate and quantify the immediate network of recent
                      follower or friend relationships amongst a group of
                      people.
                    </p>
                    <ol>
                      <li>
                        Select whether you want to pull followers or friends
                        data.
                      </li>
                      <li>
                        Select the type of sampling you want from your data (The
                        tool samples from a larger pool of recent data).
                      </li>
                      <li>
                        Upload a .csv file of names of people you wish to
                        analyze (Note: do not include the '@' symbol in twitter
                        handle names).
                      </li>
                    </ol>
                  </div>
              ...
              <ParseMenu
                {...this.state}
                changeSampling={this.onChangeSample}
                changeParser={this.onChangeParser}
              />,
              <br />,
              <FileParser
                key={i}
              ...
              <Description
                text={
                  <div>
                    <p>
                      info: This function will find mutual followers and friends
                      amongst a group of people, but will start with the input
                      of a single user handle. This means you will not be able
                      to select the group of users you want to analyze, however,
                      this is the purpose of the single user function. You will
                      be able to speculate and quantify the network of most
                      recent followers or friends a user is reaching.
                    </p>
                    <ol>
                      <li>
                        Select the type of sampling you want from your data (The
                      ...
                      <li>
                        Second, Select whether you want to pull followers or
                        friends data.
                      </li>
                      <li>
                        Third, enter the user handle you want to analyze (Note:
            ...
            [
              <Description
                text={
                  <div>
                    <p>
                      info: This function will find user information for up to
                      50,000 friends or followers of a single user.
                    </p>
                  </div>
                }
              />,
